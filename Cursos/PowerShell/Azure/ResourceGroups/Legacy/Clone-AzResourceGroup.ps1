<#
.WARNING
    Ejemplo heredado y de alto impacto. Puede crear recursos, copiar discos y
    generar costos. Revise el codigo completo y pruebe en una suscripcion de
    laboratorio antes de usarlo. Se conserva con fines historicos y educativos.

.SYNOPSIS
    Clones Azure V2 (ARM) resources from one resource group into a new resource group in the same Azure Subscriptions 
    Requires Az module version 6.7 or later.
...
.EXAMPLE
   .\Clone-AzresourceGroup.ps1 -ResourceGroupName 'CONTOSO' -NewResourceGroupName 'NEWCONTOSO'
...
.EXAMPLE
   .\Clone-AzresourceGroup.ps1 -ResourceGroupName 'CONTOSO' -NewResourceGroupName 'NEWCONTOSO' -NewLocation 'westus' -resume 
   Clones Resource Group CONTOSO as new resource group NEWCONTOSO in new location West US
.EXAMPLE
   .\Clone-AzresourceGroup.ps1 -ResourceGroupName 'CONTOSO' -NewResourceGroupName 'NEWCONTOSO' -Environment 'AzureUSGovernment'
...
if ((Get-Module Az).Version -lt "2.7") {
   Write-warning "Old version of Azure PowerShell module  $((Get-Module Az).Version.ToString()) detected.  Minimum of 2.7 required. Run Update-Module Az"
   BREAK
}
...
if(! $resume)
{
    ...
    if($NewLocation)
    {
        ...
        if(! $location) 
        {
            ...
            $location = (Get-Azlocation | Where-Object { $_.Providers -eq 'Microsoft.Compute'} | Select-Object DisplayName, Providers | Out-GridView -Title "Select Azure Resource Group Location" -OutputMode Single).location
        }
    }
    ...
    foreach($srcStorageAccountObj in $srcStorageAccounts)
    {
        ...
        if($Encryption) 
        {
            ...
            # Remarked for newer modules.  This was required prior to Az.Storage 5.0.2
    ...
    { 
	    ...
	    {
            # if it isn't a new location/region, use New-AzDiskConfig -CreateOption Copy and the resource ID of the source MD
            ...
            try
	        {
        	    $mdiskconfig = New-AzDiskConfig -SkuName $srcSkuName -Location $location  -CreateOption Copy -SourceResourceId $srcMDid
...
else # if  resume
{
    ...
    # check for valid source resource group
    if(-not ($targetResourceGroup = Get-AzResourceGroup  -ResourceGroupName $NewResourceGroupName)) 
    {
...
if($resourceGroupVMs.storageprofile.osdisk.manageddisk -and $newLocation -and $location -ne $srcLocation)
 {
    ...
    foreach($mdObj in $VHDstorageObjects|Where-Object{$_.srcSkuName -ne 'NULL'})
    {
        ...
        Get-AzDisk | Where-Object{$_.Name -eq $srcMDname} | Revoke-AzDiskAccess | Out-Null
